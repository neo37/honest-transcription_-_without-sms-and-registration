...
def process_payment(request):
    ...
    try:
        ...
        # Пока заглушка - все значения нулевые
        
        # Помечаем как оплачено по IP и UUID
        ip_counter = IPUploadCount.get_or_create_for_ip(ip_address)
        uuid_counter = UUIDUploadCount.get_or_create_for_uuid(user_uuid)
        ...
        # Логируем оплату
        logger.info(f"Оплата обработана: IP={ip_address}, UUID={user_uuid}")
        
        return JsonResponse({
            'success': True,
            'message': 'Оплата успешно обработана',
            'payment_id': 'test_payment_' + str(uuid.uuid4())[:8]  # Временный ID
        })
