...
from .models import Transcription, Screenshot, IPUploadCount, UUIDUploadCount
...
@admin.register(UUIDUploadCount)
class UUIDUploadCountAdmin(admin.ModelAdmin):
    list_display = (
        'uuid', 'upload_count', 'is_paid', 'last_upload_at', 'created_at'
    )
    list_filter = ('is_paid', 'last_upload_at', 'created_at')
    search_fields = ('uuid',)
    readonly_fields = ('uuid', 'upload_count', 'last_upload_at', 'created_at')
    fieldsets = (
        ('Информация', {
            'fields': ('uuid', 'upload_count', 'is_paid', 'last_upload_at', 'created_at')
        }),
    )
    actions = ['mark_as_paid', 'reset_upload_count']
    
    def mark_as_paid(self, request, queryset):
        """Пометить как оплачено"""
        count = queryset.update(is_paid=True)
        self.message_user(request, f'Помечено как оплачено: {count}')
    mark_as_paid.short_description = "Пометить как оплачено"
    
    def reset_upload_count(self, request, queryset):
        """Сбросить счетчик загрузок"""
        count = queryset.update(upload_count=0)
        self.message_user(request, f'Счетчики сброшены: {count}')
    reset_upload_count.short_description = "Сбросить счетчики"


@admin.register(IPUploadCount)
class IPUploadCountAdmin(admin.ModelAdmin):
    list_display = (
        'ip_address', 'upload_count', 'is_paid', 'last_upload_at', 'created_at'
    )
    list_filter = ('is_paid', 'last_upload_at', 'created_at')
    search_fields = ('ip_address',)
    readonly_fields = ('ip_address', 'upload_count', 'last_upload_at', 'created_at')
    fieldsets = (
        ('Информация', {
            'fields': ('ip_address', 'upload_count', 'is_paid', 'last_upload_at', 'created_at')
        }),
    )
    actions = ['mark_as_paid', 'reset_upload_count']
