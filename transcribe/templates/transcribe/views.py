...
def retranscribe(request, transcription_id):
    ...
    try:
        ...
        # Проверяем временные файлы, которые могли остаться
        ...
        possible_paths = [
            ...
            os.path.join(settings.MEDIA_ROOT, 'uploads', str(transcription.id), transcription.filename),
        ]
        
        # Также проверяем все временные файлы с таким ID
        ...
        # Если файл не найден, пробуем найти по паттерну
        if not original_file_path:
            import glob
            for pattern in [
                os.path.join(temp_dir, f'*{transcription.id}*'),
                os.path.join('/tmp', f'*{transcription.id}*'),
            ]:
                matches = glob.glob(pattern)
                # Ищем файл с правильным расширением
                file_ext = os.path.splitext(transcription.filename)[1]
                for match in matches:
                    if os.path.splitext(match)[1] == file_ext and os.path.isfile(match):
        ...
        if not original_file_path:
            ...
            return JsonResponse({
                'error': 'Оригинальный файл не найден на сервере. Перетранскрибация возможна только для недавно загруженных файлов, которые еще хранятся во временной директории.'
            }, status=404)
